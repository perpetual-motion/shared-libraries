...
export class Record<TKey extends string, TElement> implements Record<TKey, TElement> {
  [key: string]: TElement;
}
...
export function when<T>(value: T, onTrue: (value: NonNullable<T>) => void, onFalse: () => void = () => { /* */ }) {
  return value ? onTrue(value as NonNullable<T>) : onFalse();
}
