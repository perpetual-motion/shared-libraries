...
export interface EventOptions<TOutput = any> {
  ...
  descriptors?: Record<string, string | Array<string>>
  once?: boolean;
}
...
export abstract class Emitter {
  ...
  subscribe(): void
  subscribe<T >(...subscribers: Array<string>) : Promise<void>
  subscribe<T extends Record<string,any>>(...subscribers: Array<Subscription<T>>) : void
  subscribe<T extends Record<string,any>>(...subscribers: Array<string>|Array<Subscription<T>>) : void |Promise<void>{
    ...
    for (const each of subscribers as Array<ArbitraryObject>) {
  ...
  wait(eventExpression: string, timeout?: number) : Promise<void> {
    return new Promise<void>((e:()=>void)=>this.on(eventExpression, e));
    //    const result = new ManualPromise();
    //    this.on(eventExpression, ()=> result.resolve());
    //    return result;
  }
}
